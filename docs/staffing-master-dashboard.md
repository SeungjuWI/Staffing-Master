...
- 남은 시트 측 정리 필요 사항 — **2026-07-30 재실측으로 갱신**
  - ~~Zest 공고코드 불일치(`ZE3501/3502` ↔ `ZE3401/3402`)~~ → **해결됨**. Matching Status `VN Code` 열에 `ZE3501`·`ZE3502` 등재 확인 (`YD1903`·`YD1904` 도 등재됨). Matching Status 코드 60건 전부 JD EXECUTION 원장에 존재 = 불일치 0건
  - Matching Status 미등재 진행 중 공고는 **3건만 남음** (진행 중 32건 중)

| 공고 | 회사 | JD EXECUTION Headcount | 영향 |
| --- | --- | --- | --- |
| JIC3302 | JIC Global | 1 | Headcount 폴백으로 TO=1 — 표시는 정상 |
| **CR3601** | CREATUS | **공란** | TO 없음 → 오픈 공고 TO 합·충원율에서 제외, 판정은 TO=1 가정 (지원 9건) |
| **ET3701** | Electerior | **공란** | 위와 동일 (지원 7건) |

**조치**: Matching Status 에 3건 등재(또는 최소한 JD EXECUTION Headcount 기입) — CR3601·ET3701 은 두 원장 모두 비어 있어 TO 가 아예 없다.
...
## 7-3. FYI 직접 지원의 공고 귀속 실측 (2026-07-30)

salarymap `jobs.source_id`(공고코드)가 비어 있는 FYI 공고가 6건 있으나, **지원이 붙은 4건은 전부 정상 귀속된다** (제목 정확일치 / 회사+제목 유사 폴백이 작동):

| FYI 공고 | 귀속 코드 | 지원 | 매칭 경로 |
| --- | --- | --- | --- |
| Zest · FULL-STACK DEVELOPER | ZE3501 | 64건 | 회사+제목 유사 |
| CREATUS · Global Content Marketer | CR3601 | 9건 | 제목 정확일치 |
| Electerior · Social Media Content (AI-based) | ET3701 | 7건 | 회사+제목 유사 |
| Zest · Nhân viên thiết kế kiến trúc & CAD | ZE3502 | 0건 | 제목 정확일치 |

- 미귀속 2건은 지원 0건 (Simco 마감 공고 · LIKELION 자사 채용) → **공고별 표에서 빠지는 지원은 없다**
- 다만 귀속이 제목 문자열에 의존한다. 제목이 바뀌거나 같은 회사에 비슷한 제목이 하나 더 생기면 조용히 끊긴다 → 근본 해결은 salarymap `ktc-sync` 가 이 4건에 `source_id`(공고코드)를 채우는 것 (대시보드 쪽 수정 아님). 상시 감시는 `/api/verify` 의 `FYI 지원 공고미귀속 (모집 중 공고)` 항목
